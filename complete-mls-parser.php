...
echo "<p>Processing MLS file: $mls_file</p>\n";

if (!file_exists($mls_file)) {
    die("<p style='color: red;'>MLS file not found: $mls_file</p>");
}
...
echo "<p>Found " . count($listings) . " listings to process.</p>\n";
...
foreach ($listings as $index => $listing) {
    ...
    if ($result['success']) {
        $created_count++;
        echo "<li style='color: green;'>✓ Created: " . esc_html($result['title']) . " (Post ID: " . $result['post_id'] . ")</li>\n";
    } else {
        $errors[] = "Failed to create listing: " . ($listing['address'] ?? 'Unknown address') . " - " . $result['error'];
        echo "<li style='color: red;'>✗ Failed: " . esc_html($listing['address'] ?? 'Unknown address') . " - " . esc_html($result['error']) . "</li>\n";
    }
...
echo "<p><strong>Successfully created:</strong> $created_count</p>\n";
