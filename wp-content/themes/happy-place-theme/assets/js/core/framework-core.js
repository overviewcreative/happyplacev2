...
(function($) {
    ...
    HPH.initComponents = function() {
        // Navigation is initialized by its own module (navigation.js)
        // No need to initialize it here to avoid race conditions
    ...
    HPH.initScrollReveal = function() {
        ...
        document.querySelectorAll('.hph-scroll-reveal').forEach(function(element) {
            scrollObserver.observe(element);
        });

        // Also handle elements with animated classes (CTA and Features sections)
        document.querySelectorAll('.hph-cta--animated, .hph-features--animated').forEach(function(element) {
            // For these sections, we need to add the 'is-visible' class to child elements
            var childObserver = new IntersectionObserver(function(entries) {
                entries.forEach(function(entry) {
                    if (entry.isIntersecting) {
                        if (element.classList.contains('hph-cta--animated')) {
                            element.classList.add('is-visible');
                        } else if (element.classList.contains('hph-features--animated')) {
                            // For features, add is-visible to individual feature items with stagger
                            var features = element.querySelectorAll('.hph-feature');
                        ...
                        childObserver.unobserve(element);
                    }
                });
            }, {
                threshold: 0.1,
